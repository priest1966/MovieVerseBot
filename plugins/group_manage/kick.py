    ...
    if user.status in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER):
        if len(message.command) > 1:
            ...
            for member in client.get_chat_members(message.chat.id):
                if member.user.status in input_str and member.status not in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER):
                    try:
...
def dkick(client, message):
    ...
    if user.status in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER):
        ...
        count = 0
        for member in client.get_chat_members(message.chat.id):
            if member.user.is_deleted and member.status not in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER):
                try:
                    client.ban_chat_member(message.chat.id, member.user.id, int(time() + 45))
                    count += 1
                    sleep(1)
...
def instatus(client, message):
    ...
    status_report = (
        ...
        f"Within Month - {within_month}\nLong Time Ago - {long_time_ago}\n\n"
        f"Deleted Account - {deleted_acc}\nBot - {bot}\nUnCached - {uncached}"
    )

    chat_type = message.chat.type
    if chat_type == enums.ChatType.CHANNEL:
        sent_message.edit(status_report)
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        user = client.get_chat_member(message.chat.id, message.from_user.id)
        if user.status in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER) or user.user.id in ADMINS:
            sent_message.edit(status_report)
        else:
            sent_message.edit("You are not an administrator in this chat.")
